 ...
 * Testing:
 ...
 a b
 По коду:
 1) не используйте интерфейс Observable - это плохое решение, потому что сразу же теряется типизация кода. Для Java 1.0 такой вариант был неплохим решением (потому что там cast’ы приходилось ставить везде), после введения generics такое решение уже не может считаться удовлетворительным
 ...
 + 3) Класс SelectionModel implements Observable, но нигде не вызывает notifyObservers. Зачем тогда такое наследование?
 4) Поля типа Observable - это очень плохой способ реализации паттерна “Listener”. Во-первых, такой подход вообще нерасширяем (например, нельзя listener’у добавить аргумент), во-вторых, абсолютно не понятен другим людям (т.к. все привыкли к другой реализации Listener’ов)
